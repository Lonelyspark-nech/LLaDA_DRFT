...
class LLM2Vec(nn.Module):
    ...
    def from_pretrained(
        ...
        model = model_class.from_pretrained(base_model_name_or_path,torch_dtype="bfloat16",attn_implementation="eager", trust_remote_code=True) #**kwargs)#trust_remote_code=False)
    ...
    def tokenize(self, texts):
        ...
        # print("input_ids shape:", original["input_ids"].shape)
        # print("input_ids:", original["input_ids"])
        # print("attention_mask:", original["attention_mask"])
    ...
    def forward(self, sentence_feature: Dict[str, Tensor],is_q=False):
        ...
        if hasattr(reps, "last_hidden_state"):
            last_hidden_state = reps.last_hidden_state
        else:
            last_hidden_state = reps.hidden_states[-1]
        return self.get_pooling(sentence_feature, last_hidden_state,is_q=is_q)


    def get_pooling(self, features, last_hidden_states, is_q=False):  # All models padded from left
    ...
    def _encode(
        ...
        features = self.tokenize(
            [sentence for sentence in sentences_batch]
        )
